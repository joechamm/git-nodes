...
module.exports = function(RED) {
  ...
  function odbobject_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        OdbObject_method = nodegit.OdbObject[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        OdbObject_method.apply(OdbObject_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = OdbObject_method.apply(OdbObject_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('odbobject_node',odbobject_node);

  odbobject_node.prototype.close = function () {
    console.log('odbobject_node closed');
  };
}
