...
module.exports = function(RED) {
  ...
  function repo_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Repo_method = nodegit.Repo[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Repo_method.apply(Repo_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Repo_method.apply(Repo_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('repo_node',repo_node);

  repo_node.prototype.close = function () {
    console.log('repo_node closed');
  };
}
