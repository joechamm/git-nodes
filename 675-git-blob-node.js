...
module.exports = function(RED) {
  ...
  function blob_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Blob_method = nodegit.Blob[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Blob_method.apply(Blob_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Blob_method.apply(Blob_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('blob_node',blob_node);

  blob_node.prototype.close = function () {
    console.log('blob_node closed');
  };
}
