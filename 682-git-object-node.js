...
module.exports = function(RED) {
  ...
  function object_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Object_method = nodegit.Object[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Object_method.apply(Object_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Object_method.apply(Object_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('object_node',object_node);

  object_node.prototype.close = function () {
    console.log('object_node closed');
  };
}
