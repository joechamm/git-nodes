...
module.exports = function(RED) {
  ...
  function index_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Index_method = nodegit.Index[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Index_method.apply(Index_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Index_method.apply(Index_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('index_node',index_node);

  index_node.prototype.close = function () {
    console.log('index_node closed');
  };
}
