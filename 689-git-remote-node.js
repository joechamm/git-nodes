...
module.exports = function(RED) {
  ...
  function remote_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Remote_method = nodegit.Remote[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Remote_method.apply(Remote_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Remote_method.apply(Remote_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('remote_node',remote_node);

  remote_node.prototype.close = function () {
    console.log('remote_node closed');
  };
}
