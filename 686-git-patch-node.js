...
module.exports = function(RED) {
  ...
  function patch_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Patch_method = nodegit.Patch[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Patch_method.apply(Patch_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Patch_method.apply(Patch_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('patch_node',patch_node);

  patch_node.prototype.close = function () {
    console.log('patch_node closed');
  };
}
