...
module.exports = function(RED) {
  ...
  function treebuilder_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        TreeBuilder_method = nodegit.TreeBuilder[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        TreeBuilder_method.apply(TreeBuilder_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = TreeBuilder_method.apply(TreeBuilder_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('treebuilder_node',treebuilder_node);

  treebuilder_node.prototype.close = function () {
    console.log('treebuilder_node closed');
  };
}
