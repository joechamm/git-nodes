...
module.exports = function(RED) {
  ...
  function convenienthunk_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        ConvenientHunk_method = nodegit.ConvenientHunk[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        ConvenientHunk_method.apply(ConvenientHunk_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = ConvenientHunk_method.apply(ConvenientHunk_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('convenienthunk_node',convenienthunk_node);

  convenienthunk_node.prototype.close = function () {
    console.log('convenienthunk_node closed');
  };
}
