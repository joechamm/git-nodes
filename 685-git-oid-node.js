...
module.exports = function(RED) {
  ...
  function oid_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Oid_method = nodegit.Oid[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Oid_method.apply(Oid_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Oid_method.apply(Oid_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('oid_node',oid_node);

  oid_node.prototype.close = function () {
    console.log('oid_node closed');
  };
}
