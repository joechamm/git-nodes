...
module.exports = function(RED) {
  ...
  function odb_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Odb_method = nodegit.Odb[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Odb_method.apply(Odb_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Odb_method.apply(Odb_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('odb_node',odb_node);

  odb_node.prototype.close = function () {
    console.log('odb_node closed');
  };
}
