...
module.exports = function(RED) {
  ...
  function treeentry_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        TreeEntry_method = nodegit.TreeEntry[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        TreeEntry_method.apply(TreeEntry_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = TreeEntry_method.apply(TreeEntry_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('treeentry_node',treeentry_node);

  treeentry_node.prototype.close = function () {
    console.log('treeentry_node closed');
  };
}
