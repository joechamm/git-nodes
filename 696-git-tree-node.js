...
module.exports = function(RED) {
  ...
  function tree_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Tree_method = nodegit.Tree[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Tree_method.apply(Tree_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Tree_method.apply(Tree_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('tree_node',tree_node);

  tree_node.prototype.close = function () {
    console.log('tree_node closed');
  };
}
