...
module.exports = function(RED) {
  ...
  function threads_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Threads_method = nodegit.Threads[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Threads_method.apply(Threads_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Threads_method.apply(Threads_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('threads_node',threads_node);

  threads_node.prototype.close = function () {
    console.log('threads_node closed');
  };
}
