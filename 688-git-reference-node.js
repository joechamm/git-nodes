...
module.exports = function(RED) {
  ...
  function reference_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Reference_method = nodegit.Reference[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Reference_method.apply(Reference_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Reference_method.apply(Reference_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('reference_node',reference_node);

  reference_node.prototype.close = function () {
    console.log('reference_node closed');
  };
}
