...
module.exports = function(RED) {
  ...
  function tag_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Tag_method = nodegit.Tag[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Tag_method.apply(Tag_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Tag_method.apply(Tag_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('tag_node',tag_node);

  tag_node.prototype.close = function () {
    console.log('tag_node closed');
  };
}
