...
module.exports = function(RED) {
  ...
  function submodule_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Submodule_method = nodegit.Submodule[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Submodule_method.apply(Submodule_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Submodule_method.apply(Submodule_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('submodule_node',submodule_node);

  submodule_node.prototype.close = function () {
    console.log('submodule_node closed');
  };
}
