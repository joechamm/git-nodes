...
module.exports = function(RED) {
  ...
  function commit_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Commit_method = nodegit.Commit[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Commit_method.apply(Commit_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Commit_method.apply(Commit_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('commit_node',commit_node);

  commit_node.prototype.close = function () {
    console.log('commit_node closed');
  };
}
