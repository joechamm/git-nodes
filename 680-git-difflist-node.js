...
module.exports = function(RED) {
  ...
  function difflist_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        DiffList_method = nodegit.DiffList[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        DiffList_method.apply(DiffList_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = DiffList_method.apply(DiffList_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('difflist_node',difflist_node);

  difflist_node.prototype.close = function () {
    console.log('difflist_node closed');
  };
}
