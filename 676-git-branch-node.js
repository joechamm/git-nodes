...
module.exports = function(RED) {
  ...
  function branch_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Branch_method = nodegit.Branch[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Branch_method.apply(Branch_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Branch_method.apply(Branch_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('branch_node',branch_node);

  branch_node.prototype.close = function () {
    console.log('branch_node closed');
  };
}
