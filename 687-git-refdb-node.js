...
module.exports = function(RED) {
  ...
  function refdb_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        RefDb_method = nodegit.RefDb[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        RefDb_method.apply(RefDb_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = RefDb_method.apply(RefDb_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('refdb_node',refdb_node);

  refdb_node.prototype.close = function () {
    console.log('refdb_node closed');
  };
}
