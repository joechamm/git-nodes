...
module.exports = function(RED) {
  ...
  function signature_node(n) {
    ...
    this.on('input', function (msg) {
      ...
      var newmsg,
        Signature_method = nodegit.Signature[node.git_command],
        has_callback = false,
        resolved_arguments = [],
        cb_string,
        cb_args;
      ...
      if (has_callback) {
        ...
        Signature_method.apply(Signature_method,resolved_arguments);
      } else {
        newmsg.payload.return_value = Signature_method.apply(Signature_method,resolved_arguments);
        node.send(newmsg);
      }
    });
  }

  RED.nodes.registerType('signature_node',signature_node);

  signature_node.prototype.close = function () {
    console.log('signature_node closed');
  };
}
